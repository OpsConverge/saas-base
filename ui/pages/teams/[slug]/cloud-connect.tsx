  ...
  useEffect(() => {
    ...
    socket.on('deploymentUpdate', (update) => {
      ...
      if (update.status === 'CREATE_COMPLETE') {
        ...
        setLoading(false); // Stop loading when deployment is complete
      } else if (update.status === 'FAILED') {
        toast.error(`Deployment failed: ${update.message}`);
        setLoading(false); // Stop loading if deployment fails
      }
    });
    ...
    socket.on('disconnect', () => {
      ...
      setLoading(false); // Stop loading on disconnection
    });
